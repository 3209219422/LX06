...
namespace DAL
{
    ...
    {
        private Comment() { }
        private static Comment _insance = new Comment();
        public static Comment Insance
        {
            get
            {
                return _insance;
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@CommentContect,CommentTime=@CommentContent where commentID=@commentID";
